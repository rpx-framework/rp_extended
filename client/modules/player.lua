function PlayerObject(data)
    ...
    player.AddItem = function(item, amount, data)
        return Inventory.AddItem(player.source, item, amount, data)
    end
    
    player.RemoveItem = function(item, amount, data)
        return Inventory.RemoveItem(player.source, item, amount, data)
    end
    
    player.Search = function(item, data)
        return Inventory.Search(player.source, item, data)
    end
    
    -- Status
    
    player.GetStatus = function(name)
        return player.status[name]
    end
    
    player.AddStatus = function(name, amount)
        if not tonumber(amount) or amount < 1 then return player.status[name] end
        if not player.status[name] then 
            player.status[name] = amount 
        else 
            player.status[name] = player.status[name] + amount
        end
        return player.status[name]
    end
    
    player.RemoveStatus = function(name, amount)
        if not tonumber(amount) or amount < 1 then return player.status[name] end
        if not player.status[name] then 
            player.status[name] = -amount 
        else 
    ...
    player.SetStatus = function(name, amount)
